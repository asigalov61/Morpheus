...
TMIDIX.Tegridy_Any_Pickle_File_Writer(INTS_f1, '/notebooks/Morpheus_INTS_128x128')


# In[ ]:


INTS_f1 = TMIDIX.Tegridy_Any_Pickle_File_Reader('/notebooks/Morpheus_INTS_128x128')
...
for i in tqdm(INTS_f1):
    ...
    if min(i) >= 0:
        ...
        if i[4] > avg_vel: 
            ...
            train_data1.extend([(128*128) + (256 * i[3])+i[2]])

        pe = i
  
    if i == [-1, -1, -1, -1, -1]: # Intro
        train_data1.extend([(128*128)+(256 * 11)-3])

    if i == [-2, -2, -2, -2, -2]: # Outro
        train_data1.extend([(128*128)+(256 * 11)-2])

    if i == [-3, -3, -3, -3, -3]: # End
        train_data1.extend([(128*128)+(256 * 11)-1])
